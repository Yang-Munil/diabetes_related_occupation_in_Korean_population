...
# 데이터 불러오기
hn18_all <- read_sas("C:/rproject/startr/hn16_all.sas7bdat", NULL)
...
hn18_all_sub <- hn18_all %>% select(year, sex, age, occp, HE_BMI, HE_DM, BS3_1, educ, marri_1, ainc, incm, EC_wh)
hn17_all_sub <- hn17_all %>% select(year, sex, age, occp, HE_BMI, HE_DM, BS3_1, educ, marri_1, ainc, incm, EC_wh)
hn16_all_sub <- hn16_all %>% select(year, sex, age, occp, HE_BMI, HE_DM, BS3_1, educ, marri_1, ainc, incm, EC_wh)
...
# EC_wh (9 모름, 결측치 제거하면서 삭제됨.)

hn_all <- transform(hn_all, EC_wh_1 = ifelse(EC_wh == 1, 1, 0), EC_wh_2 = ifelse(EC_wh == 2, 1, 0), EC_wh_8 = ifelse(EC_wh == 8, 1, 0))
...
hn_all$ainc_band[hn_all$ainc>=400]=2
hn_all$ainc_band[hn_all$ainc>=600]=3
...
hn_all <- subset(hn_all, select = -c(year, sex, age, occp, BS3_1, educ, marri_1, ainc, incm, EC_wh, HE_BMI, age_band, ainc_band, HE_BMI_band))
...
hn_all_der <- rbind(hn18_all_sub, hn17_all_sub, hn16_all_sub)
hn_all_der <- hn_all[complete.cases(hn_all[, c("HE_BMI", "HE_DM", "incm", "ainc", "occp")]), ]

hn_all_der <- hn_all_der[!(hn_all_der$age < 19),]
